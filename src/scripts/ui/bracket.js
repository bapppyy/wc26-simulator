...
// ── Standalone bracket tree builder ──────────────────────────────────────────
// Takes any sim data object (state.LAST or a SIMS entry) and returns a
// .bkt-tree DOM element ready to append. Does NOT touch any global DOM.
export function renderBracketTree(simData) {
  const r32 = simData.r32 || [];
  const r16 = simData.r16 || [];
  const qf  = simData.qf  || [];
  const sf  = simData.sf  || [];
  const fin = simData.fin || [];
  ...
  // ── Bracket ordering ──────────────────────────────────────────────────────
  // Left wing: skip-one R32→R16 pairing; consecutive R16→QF.
  // Right wing: consecutive R32→R16 pairing; row-reverse handles visual flip.
  // Append order for right wing is R32→SF so row-reverse places R32 on far
  // right and SF adjacent to the center spine.
  ...
  leftWing.appendChild(makeCol(leftR32, t('bkt.r32'), true));
  leftWing.appendChild(makeCol(leftR16, t('bkt.r16')));
  leftWing.appendChild(makeCol(leftQF,  t('bkt.qf')));
  leftWing.appendChild(makeCol(leftSF,  t('bkt.sf')));
  ...
  // Champion crown sits at the absolute top — above the Final label
  if (simData.champion) {
    const champ = document.createElement('div');
    champ.className = 'bkt-champ';
    champ.innerHTML =
      `<div style="font-size:26px">${flag(simData.champion)}</div>` +
      `<div class="bkt-champ-name">${tTeam(simData.champion)}</div>` +
      `<div class="bkt-champ-lbl">${t('bkt.champion')}</div>`;
    finalWrap.appendChild(champ);
  }

  const finLbl = document.createElement('div');
  finLbl.className = 'bkt-rnd-lbl';
  finLbl.style.cssText = 'margin-top:6px;margin-bottom:5px;border-radius:5px;width:100%;border:1px solid #e5e5e0';
  finLbl.textContent = t('bkt.final');
  ...
  if (simData.third) {
    ...
    thirdLbl.textContent = t('bkt.third');
    thirdWrap.appendChild(thirdLbl);
    thirdWrap.appendChild(matchCard(simData.third));
  ...
  rightWing.appendChild(makeCol(rightR32, t('bkt.r32'), true));
  rightWing.appendChild(makeCol(rightR16, t('bkt.r16')));
  rightWing.appendChild(makeCol(rightQF,  t('bkt.qf')));
  rightWing.appendChild(makeCol(rightSF,  t('bkt.sf')));
...
// ── Bracket Panel entry point ─────────────────────────────────────────────────
// Renders the last simulation's bracket into the dedicated #bracketEl container.
export function drawBracket() {
  drawAnnexPanel();
  const el = document.getElementById('bracketEl');
  el.innerHTML = '';
  if (!state.LAST) return;
  el.appendChild(renderBracketTree(state.LAST));
}
...
export function drawAnnexPanel() {
  ...
  const allGKeys = ['A','B','C','D','E','F','G','H','I','J','K','L'];
  const eliminated = allGKeys.filter(g => !advancing.includes(g));

  let html =
    `<div class="annex-panel"><div class="annex-title">3rd Place Scenario — Annex C/${key}</div>` +
    `<div style="font-size:11px;color:#888;margin-bottom:3px">` +
    `<span style="font-weight:600;color:#15803d;margin-right:4px">✓ Qualified:</span>` +
    advancing.map(g =>
      `<span class="gb" title="${g}: ${LAST.grpSt[g][2]?.name || '?'}">${g}:${flag(LAST.grpSt[g][2]?.name)}</span>`
    ).join('') +
    `</div>` +
    `<div style="font-size:11px;color:#888;margin-bottom:7px">` +
    `<span style="font-weight:600;color:#b45309;margin-right:4px">✗ Eliminated:</span>` +
    eliminated.map(g =>
      `<span class="gb" style="opacity:0.45" title="${g}: ${LAST.grpSt[g][2]?.name || '?'}">${g}:${flag(LAST.grpSt[g][2]?.name)}</span>`
    ).join('') +
    `</div>` +
    `<div class="annex-title">R32 Dynamic Matchups</div><div class="annex-grid">`;
